        ...
        {[1, 2, 3].map((n) => (
          <span
            ...
            className={`h-2 w-2 rounded-full ${
              n <= filled ? "bg-brand" : "bg-slate-200"
            }`}
          />
        ))}
      </span>
...
export default function SummaryCards({ summary }: SummaryCardsProps) {
  ...
  return (
    <div className="space-y-8">
      <div className="rounded-xl border border-brand/40 bg-brand/[0.06] shadow-sm shadow-brand/10 px-4 py-3 text-center">
      ...
      <blockquote className="relative rounded-2xl bg-white border border-slate-200/80 shadow-lg px-6 py-6 md:px-10 md:py-8">
        <span
          className="absolute left-0 top-6 bottom-6 w-1 rounded-full bg-brand"
          aria-hidden
        />
          ...
          {summary.opportunities.map((item, index) => (
            ...
            >
              ...
              <div className="relative flex flex-wrap gap-x-5 gap-y-2 mt-5 pt-4 border-t border-slate-100">
                <DotMeter label="Impact" value={item.impact} />
                <DotMeter label="Effort" value={item.effort} />
              </div>
            </article>
          ))}
        </div>
